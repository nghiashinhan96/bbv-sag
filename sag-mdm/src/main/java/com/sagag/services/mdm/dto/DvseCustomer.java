...
public class DvseCustomer {
  ...
  public String toString() {
    return MoreObjects.toStringHelper(this).add("traderId", traderId).add("customerId", customerId).add("customerName", customerName).add("cdate", cdate)
        .add("udate", udate).add("uuser", uuser).add("memo", memo)
        .add("usersInfos", Joiner.on(", ").join(MoreObjects.firstNonNull(usersInfos, ImmutableList.of())))
        .add("modules", Joiner.on(", ").join(MoreObjects.firstNonNull(modules, ImmutableList.of()))).toString();
  }
}
