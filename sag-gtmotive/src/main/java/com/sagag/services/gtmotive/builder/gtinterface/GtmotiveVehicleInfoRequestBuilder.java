...
public class GtmotiveVehicleInfoRequestBuilder extends AbstractGtmotiveRequestBuilder {

  @Setter
  private GtmotiveVehicleInfoCriteria criteria;
  ...
  private static String contentsBuilder(GtmotiveVehicleInfoCriteria criteria, boolean readMode) {
  ...
  private static String authenticationData(GtmotiveVehicleInfoCriteria criteria) {
  ...
  private static String estimate(GtmotiveVehicleInfoCriteria criteria, boolean readMode) {
    final StringBuilder estimateBuilder = new StringBuilder();
    estimateBuilder.append("<estimate>");
    estimateBuilder.append("<operation value=\"").append(readMode ? "read" : "create")
        .append("\"/>");
  ...
  private static String vehicleInfo(GtmotiveVehicleInfoCriteria criteria, String registrationNumber) {
    final StringBuilder vehBuilder = new StringBuilder("<vehicleInfo>");

    if (!StringUtils.isBlank(criteria.getModifiedVin())) {
      vehBuilder.append("<registrationNumber value=\"").append(registrationNumber)
          .append(ATTR_LOCKED_FALSE);
      vehBuilder.append("<vin value=\"").append(criteria.getModifiedVin())
      .append(ATTR_LOCKED_FALSE);
    } else {
