...
/**
 * The mapper class to map AX definition of payment type to Connect payment for sales with KSL.
 *
 */
@Component
@Slf4j
public class AxPaymentTypeForSalesWithKSLTranslator
    implements AxDataTranslator<String, PaymentMethodType> {
  ...
  public PaymentMethodType translateToConnect(final String axPaymentType) {
    ...
    if (StringUtils.isBlank(axPaymentType)) {
      ...
      throw new AxCustomerException(AxCustomerException.AxCustomerErrorCase.AC_PAYMENT_TYPE_001,
          msg);
    }

    final PaymentMethodType paymentMethodType =
        paymentTypeMapper.getAxPaymentTypeMapperForSalesWithKSLTranslator()
