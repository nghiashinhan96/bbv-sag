...
@Slf4j
@Transactional
public class CustomerMigrationItemProcessorV2
    implements ItemProcessor<CsvCustomerInfoData, ImportedCustomerData>, IKlausCustomerMapper {
  ...
  public ImportedCustomerData process(CsvCustomerInfoData item) throws Exception {
    ...
    if (!axCustomerInfo.filter(filterKlausCustomer()).isPresent()) {
      log.warn("This customer nr = {} is not belong to Klaus affiliate", data.getCustomerNr());
      return null;
    }
  ...
  private List<PermissionEnum> buildCollectionPermissions() {
    final List<CollectionPermission> maxPermissionForCollection =
        collPermissionRepo.findByCollectionShortName(SupportedAffiliate.KLAUS.getAffiliate());
    return findPermissions(SupportedAffiliate.KLAUS.getAffiliate(), maxPermissionForCollection, true)
        .stream()
