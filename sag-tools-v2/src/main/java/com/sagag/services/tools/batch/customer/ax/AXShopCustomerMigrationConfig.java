...
@Configuration
@OracleProfile
public class AXShopCustomerMigrationConfig extends AbstractJobConfig {
  ...
  public ListItemReader<CustInfo> axCustomerItemReader() throws BatchJobException {
    File file = new File(SystemUtils.getUserDir() + "/csv/shop_customer.csv");
    final List<CsvAXCustomer> customers = CsvUtils.read(file, CsvAXCustomer.class);
    final List<CustInfo> custInfoList =
        customers.stream().map(CsvAXCustomer::getCustomerNr).map(custNr -> new CustInfo(custNr, CountryCode.AT)).collect(Collectors.toList());
