...
import com.sagag.services.tools.domain.mdm.DvseGetCustomerRequest;
import com.sagag.services.tools.domain.mdm.DvseGetCustomerResponse;
import com.sagag.services.tools.domain.mdm.DvseGetCustomerSearchResultRequest;
import com.sagag.services.tools.domain.mdm.DvseGetCustomerSearchResultResponse;
import com.sagag.services.tools.domain.mdm.DvseGetCustomerUsersRequest;
import com.sagag.services.tools.domain.mdm.DvseGetCustomerUsersResponse;
import com.sagag.services.tools.domain.mdm.DvseLoginRequest;
import com.sagag.services.tools.domain.mdm.DvseLoginResponse;
import com.sagag.services.tools.domain.mdm.DvseLogoutRequest;
import com.sagag.services.tools.domain.mdm.DvseLogoutResponse;
import com.sagag.services.tools.domain.mdm.DvseRequest;
import com.sagag.services.tools.domain.mdm.DvseSetCustomerRequest;
import com.sagag.services.tools.domain.mdm.DvseSetCustomerResponse;
import com.sagag.services.tools.domain.mdm.DvseSetCustomerUsersRequest;
import com.sagag.services.tools.domain.mdm.DvseSetCustomerUsersResponse;
import com.sagag.services.tools.exception.MdmResponseException;
import com.sagag.services.tools.mdm.wsdl.ExecuteXml;
import com.sagag.services.tools.mdm.wsdl.ExecuteXmlResponse;
import com.sagag.services.tools.mdm.wsdl.GetBackXml;
import com.sagag.services.tools.mdm.wsdl.ObjectFactory;
import com.sagag.services.tools.support.MdmUpdateMode;
import com.sagag.services.tools.utils.MdmUtils;
import com.sagag.services.tools.utils.XmlUtils;
...
public class MdmClient {
  ...
  private static final String DVSE_ERROR_MESSAGE_PATTERN = "DVSE response has error code = {0} and message = {1}";

  private static final SoapActionCallback EXECUTE_XML_SOAP_ACTION_CALLBACK = new SoapActionCallback("http://tempuri.org/IService/ExecuteXml");
  ...
  public String createOrDeleteCustomer(String sessionId, DvseCustomer customer, MdmUpdateMode mode) {
    log.info("Start create or delete customer with session id = {}, customer = {} and mode = {}", sessionId, customer, mode);
  ...
  public List<DvseCustomerUserInfo> createOrDeleteCustomerUsers(String sessionId, String customerId, List<DvseCustomerUser> users, MdmUpdateMode mode) {
    log.info("Start create or delete customer user with session id = {}, " + "customer id = {}, users = {} and mode = {}", sessionId, customerId, users, mode);
  ...
  private <T> T doExecuteXml(DvseRequest request, Class<T> responseClazz, String sessionId) {
    ...
    final ExecuteXmlResponse response = (ExecuteXmlResponse) webServiceTemplate.marshalSendAndReceive(uri, executeXml, EXECUTE_XML_SOAP_ACTION_CALLBACK);

    final GetBackXml result = response.getExecuteXmlResult().getValue();

    if (result.getCode() != SUCCESS_RESPONSE_CODE) {
      final String message = MessageFormat.format(DVSE_ERROR_MESSAGE_PATTERN, result.getCode(), result.getMessage().getValue());
      log.error(message);
  ...
  public List<DvseCustomerSearchResult> getCustomerSearchResult(String sessionId, String traderId, String searchId, String searchString) {
    ...
    return doExecuteXml(request, DvseGetCustomerSearchResultResponse.class, sessionId).getCustomerSearchResults();
  }
