...
export class FavoriteVehicleEvent extends MetadataLogging {
    eventType = AnalyticEventType.VEHICLE_SEARCH_EVENT;
    vehIdResult: string;
    vehNameResult: string;
    ...
    constructor(metadata: MetadataLogging | any, user: UserDetail, data: any, basketItemSource?: BasketItemSource) {
        super(metadata, user);
        if (data) {
            this.vehIdResult = data.vehIdResult;
            this.vehNameResult = data.vehNameResult;
    ...
    toEventRequest() {
        ...
        return {
            ...request,
            veh_vehicle_id_result: this.vehIdResult,
            veh_vehicle_name_result: this.vehNameResult,
