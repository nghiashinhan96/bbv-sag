...
@Component
@Slf4j
public class EanArticleSearchQueryBuilder
  ...
  public SearchQuery buildQuery(KeywordArticleSearchCriteria criteria, Pageable pageable,
    ...
    Assert.hasText(pnrn, "The EAN code should not be null or blank");
    ...
    final BoolQueryBuilder queryBuilder =
        applyCommonQueryBuilder(criteria.getAffNameLocks(), criteria.isSaleOnBehalf()).apply(PartArticleSearchQueryBuilder
            .partQueryBuilder(pnrn, ArticlePartType.EAN, criteria.isUsePartsExt()));
    ...
    final NativeSearchQueryBuilder searchQueryBuilder = ArticleQueryUtils.nativeQueryBuilder(
        pageable, indices).withQuery(queryBuilder);
