...
yarn run build--ax-cz-pre

echo "Packaging Ax Czech web"
cd ../../../../
mvn clean install -Dmaven.test.skip=true -Ppre -Dbuild.customer=ax-cz
cd ../
...
cd eshop-web/src/main/resources/client
yarn run build--ehaxcz-pre

echo "Packaging Wholesaler web"
cd ../../../../
mvn -U clean install -Dmaven.test.skip=true -Ppre -Dbuild.customer=ax-cz
cd ../
