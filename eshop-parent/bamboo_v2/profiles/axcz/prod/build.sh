...
yarn run build--ax-cz-prod

echo "Packaging Ax Czech web"
cd ../../../../
mvn clean install -Dmaven.test.skip=true -Pprod -Dbuild.customer=ax-cz
cd ../
...
cd eshop-web/src/main/resources/client
yarn run build--ehaxcz-prod

echo "Packaging Wholesaler web"
cd ../../../../
mvn -U clean install -Dmaven.test.skip=true -Pprod -Dbuild.customer=ax-cz
cd ../
...
do
  ...
  curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$tomcat/manager/text/undeploy?path=/$bamboo_ax_cz"
  curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$tomcat/manager/text/undeploy?path=/$bamboo_bo"
  curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$tomcat/manager/text/undeploy?path=/$bamboo_eh_cz"
done
