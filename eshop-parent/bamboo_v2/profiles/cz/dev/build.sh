...
yarn run build--st-cz-dev

echo "Packaging Stakis Czech web"
cd ../../../../
mvn clean install -Dmaven.test.skip=true -Pdev -Dbuild.customer=cz
cd ../
...
curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$bamboo_sag_tomcat_host/manager/text/undeploy?path=/$bamboo_st_cz"
curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$bamboo_sag_tomcat_host/manager/text/undeploy?path=/$bamboo_eh_st_cz"
