...
yarn run build--st-cz-prod

echo "Packaging Stakis Czech web"
cd ../../../../
mvn -U clean install -Dmaven.test.skip=true -Pprod -Dbuild.customer=cz
cd ../
...
yarn run build--cz-prod
cd ../../../../
pwd
ls
mvn clean install -Dmaven.test.skip=true -Pprod -Dbuild.customer=cz
cd ../
...
do
	...
	then
		...
		curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$tomcat/manager/text/undeploy?path=/$bamboo_st_cz"
		curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$tomcat/manager/text/undeploy?path=/$bamboo_eh_st_cz"
