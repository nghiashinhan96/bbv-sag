...
yarn run build--wt-sb-dev

echo "Packaging WINT Serbia web"
cd ../../../../
mvn clean install -Dmaven.test.skip=true -Pdev -Dbuild.customer=sb
cd ../
...
curl -u $bamboo_sag_tomcat_user:$bamboo_sag_tomcat_password "$bamboo_sag_tomcat_host/manager/text/undeploy?path=/$bamboo_wt_sb"
