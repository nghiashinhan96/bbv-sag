...
public class CouponReValidateCodeStepTest {
  ...
  public void handle_shouldClearCacheAndResetDiscount_givenInvalidData() throws Exception {
    ...
    data.setCart(cart);
    when(couponCodeValidator.isInValid(data)).thenReturn(true);
    doNothing().when(couponCacheService).clearCache(COUPON_CODE);
    CouponValidationData results = couponReValidateCodeStep.handle(data);
  ...
  public void handle_shouldNotClearCacheAndResetDiscount_givenValidData() throws Exception {
    ...
    CouponValidationData data = CouponValidationData.builder().coupConditions(conditions).build();
    data.setCart(cart);
    when(couponCodeValidator.isInValid(data)).thenReturn(false);
    doNothing().when(couponCacheService).clearCache(COUPON_CODE);
    CouponValidationData results = couponReValidateCodeStep.handle(data);
