...
@Component
@Slf4j
public class ShortShoppingCartCsvExporter extends ShoppingCartFileExporter
    implements StreamedCsvExporter<List<ShortShoppingCartExportItemDto>> {
   ...
   */
  @Override
  public ExportStreamedResult exportCsv(final List<ShortShoppingCartExportItemDto> items)
      throws ServiceException {

    Assert.notEmpty(items, EMPTY_CART_ITEMS_MSG);

    try (final ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      final List<String[]> csvExportList = new LinkedList<>();

      csvExportList.addAll(getShortShoppingCartItemsInfo(items));
