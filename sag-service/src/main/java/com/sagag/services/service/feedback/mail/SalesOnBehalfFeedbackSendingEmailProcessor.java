...
@Component
public class SalesOnBehalfFeedbackSendingEmailProcessor
  ...
  public void processThenSendingEmail(UserInfo user, int topicId,
    ...
    String country = countryService.searchDefaultLocaleByAffiliate(user.getAffiliateShortName()).getISO3Country();

    //@formatter:off
    FeedbackMessageCriteria emailToRecipientCriteria = FeedbackMessageCriteria.builder()
        .fromEmail(salesEmail)
        .toEmail(getDepartmentEmail(topicId, user.getAffiliateShortName()))
        ...
        .subjectParams(new String[]{ country, defaultString(createdFeedbackId.toString()),
